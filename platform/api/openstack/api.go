...
type Options struct {
	*platform.Options
	...
	// Tracks whether SecurityGroup should be deleted
...
func (a *API) CreateInstance(name, sshKeyID, userdata string) (machine *Machine, err error) {
	...
	if a.opts.SecurityGroup == "" {
		err = a.createSecurityGroup(name)
		if err != nil {
			return nil, fmt.Errorf("creating security group: %v", err)
		}
		a.opts.createdSecurityGroup = true
	}
...
func (a *API) createSecurityGroup(name string) error {
	...
	a.opts.SecurityGroup = securityGroup.ID
	...
	for _, rule := range ruleSets {
		_, err = rules.Create(a.networkClient, rules.CreateOpts{
			...
			SecGroupID:     a.opts.SecurityGroup,
...
func (a *API) DeleteSecurityGroup() error {
	return groups.Delete(a.networkClient, a.opts.SecurityGroup).ExtractErr()
}
